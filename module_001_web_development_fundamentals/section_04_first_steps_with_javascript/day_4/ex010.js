...
console.log(lucroTotal(2, 4));
console.log(lucroTotal(3, 5));

function lucroTotal(custo, valorDeVenda) {
  let imposto = 1.2;
  let valorCustoTotal = custo * imposto;
  let lucro = valorDeVenda - valorCustoTotal;

  if (custo < 0 || valorDeVenda < 0) {
    return "Valores de entrada menores que zero.";
  } else {
    return `O lucro vendendo 1000 produtos é de: R$${lucro * 1000}`;
  }
}
